...
- **基础 URL**: 
  - 生产环境: `https://video.fusionfintrade.com` （请替换为实际绑定的域名）
...
| `timestamp` | String | 是 | 请求发生的时间戳，格式必须为：`YYYY-MM-DD HH:mm:ss`。 | `2026-03-19 12:00:00` |
...
API_URL = "https://video.fusionfintrade.com/api/v1/device/getData"
...
在 Postman 中调试时，将以下代码粘贴到请求的 `Pre-request Script` 中，即可实现每次点击 Send 时自动计算并注入时间戳和签名。

```javascript
// 1. 设置你的 AppSecret (如果是其他账号，请替换这里的值)
const APP_SECRET = "DEMO_SECRET_ABC123XYZ890";

// 2. 获取格式化的当前时间 (YYYY-MM-DD HH:mm:ss)
